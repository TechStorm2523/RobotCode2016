...
//	private final double MIN_SCORE = 75; // could be used to take out low-score targets so that it doesn't even lock on to one
										// if they're all bad
...
//	private ParticleReport[] allTargets = null; // I didn't end up using a global variable of these because there was no need
	public double currentRangeToBestTarget = 0;
	public boolean trackRatherThanTransfer = false;
	
	// A structure to hold measurements of a particle (has to extend certain interfaces to be used
	// with the nivision things, which means it has to implement some functions below)
...
//        System.out.println("INITING");
	...
	/**
	 * Chooses to run either full camera tracking processes or just tranfer
	 * the image to the Driver Station, depending on the global setting.
	 * Is called periodically by a command.
	 */
	...
	{
		// this is the basic outline of what happens in the vision procession, except
		// we extract information between the last two lines using a bunch of tecniques. 
		
		// grab the image from the actual camera
		...
		// set the image to send to the Driver Station
...
//			VAL_RANGE.maxValue = (int)SmartDashboard.getNumber("Tote val max", VAL_RANGE.maxValue);
			...
			// threshold based on Hue, Saturation, and Value
			NIVision.imaqColorThreshold(binaryFrame, frame, 255, NIVision.ColorMode.HSV, HUE_RANGE, SAT_RANGE, VAL_RANGE);
			
			// show threshold for diagnostics (this image will flash on before being overwritten, but you can still see it on the DS)
...
//					par.PercentAreaToImageArea = NIVision.imaqMeasureParticle(binaryFrame, particleIndex, 0, NIVision.MeasurementType.MT_AREA_BY_IMAGE_AREA);
					...
					// FINALLY, add this complete report to our list
					particles.add(par);
				}
				
				// sort the list... I don't know why this is here, it's inefficent 
				// (unless it was sorted by the cumulative score like we kinda do below, in which case we could just use the first or last)
//				particles.sort(null);
				...
				{
					// if this one is better than the best, it is now the best
					...
					// draw bounding boxes for all, though
				...
				// diagnostics (for getting constants (tuning) used for range finding)
...
//				System.out.println(bestScore);		
	...
	private double getRangeToBestTarget()
	{
		if (currentBestTarget != null && frame != null)
		{
			...
			return TARGET_HEIGHT*size.height / (2*targetHeightPixel*Math.tan(Math.toRadians(CAMERA_FOV/2)))
					*Math.cos(Math.toRadians(CAMERA_ELEVATION)); // convert to horizontal distance
		}
		else
			return 0;
	}
	...
	/**
	 * Draw a crosshair indicator at the given position, and add it to the given Image
	 */
	...
	/**
	 * Draws a bounding box for the given particle's ParticleReport on the given Image
	 * @param particle
	 * @param frame
	 */
